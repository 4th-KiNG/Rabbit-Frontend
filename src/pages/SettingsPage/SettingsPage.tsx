...
const SettingsPage = () => {
  return (
    <>
      <h1 className="text-[2.5rem] font-bold mb-8"
      style={isMobileSettings() ? {fontSize: "1.5rem", marginBottom: "20px"} : {}}>Настройки</h1>
      <form className="max-w-[664px] w-full gap-y-11 flex flex-col" action="#">
        <div>
          <h2 className="text-[2.5rem] font-bold text-[#7F7F7F] mb-6"
          style={isMobileSettings() ? {fontSize: "1.5rem"} : {}}>Личные данные</h2>
          <div className="flex flex-wrap gap-x-8 gap-y-4 max-[475px]:gap-x-4">
            ...
            className="max-w-[200px] [&_svg]:scale-150 [&_label]:translate-x-4 
            [&_button[data-slot='trigger']]:rounded-[62px] [&_button[data-slot='trigger']]:px-4" 
            style={isMobileSettings() ? {maxWidth: "156px"} : {}}>
              {genders.map((gender) => {
        ...
        <div>
          <h2 className="text-[2.5rem] font-bold text-[#7F7F7F] mb-6">Пароль</h2>
          <div className="flex flex-wrap gap-x-8 gap-y-4 max-[475px]:gap-x-4">
